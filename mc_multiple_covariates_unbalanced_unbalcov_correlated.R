...
# this simulation assumes that the unobserved blocks are balanced;
# the two binary covariates are balanced BUT correlated
...
mc_simulations <- function(index, n, Beta){
  ...
  # Balanced case
  population_prop <- c(.4,.6)
  cormat <- matrix(c(1,.3,.3,1), nrow = 2, ncol=2)
  covariates <- draw.correlated.binary(no.row = n, 
                                       d = 2,
                                       prop.vec = population_prop,
                                       corr.mat = cormat)
  cor(covariates)
...
clusterEvalQ(cl, "require(MultiRNG)")
...
clusterExport(cl, c("draw.correlated.binary", "BFcheck", "Removecheck", "getBlockCovariates", "estimatebeta", "estimatebeta2", "logit", "sigmoid"))
...
setwd("C:/Users/amele1/Dropbox/grdpg/paper/jasa_template/supplement/mc")
filename <- paste("mc_correl_unbalanced_unbalcov_estimates_nsim", nsim, "_n", n, "_all.RData", sep="")
save.image(file = filename)
