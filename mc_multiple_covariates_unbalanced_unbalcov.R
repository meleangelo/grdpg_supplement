...
# the two binary covariates are independent and unbalanced
...
mc_simulations <- function(index, n, Beta){
  ...
  covariates[,1] <- rbinom(n,1,.4)
  covariates[,2] <- rbinom(n,1,.6)
...
results <- parLapply(cl, X=1000:nsim, function(X) mc_simulations(X, n = n, Beta = Beta))
stopCluster(cl)
...
setwd("C:/Users/amele1/Dropbox/grdpg/paper/jasa_template/supplement/mc")
filename <- paste("mc_unbalanced_unbalcov_estimates_nsim", nsim, "_n", n, "_all.RData", sep="")
save.image(file = filename)
